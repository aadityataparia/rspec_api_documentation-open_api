module RspecApiDocumentation
  module OpenApi
    class Index
      ...
      def examples
        index.examples.map { |example| OpenApi::RspecExample.new(example) }
      end
        ...
        arr.each do |securities|
          securities.each do |security, opts|
            schema = OpenApi::SecurityScheme.new(
              ...
              in: opts[:in],
              scheme: opts[:scheme],
              bearerFormat: opts[:bearer_format],
              flows: opts[:flows],
              openIdConnectUrl: opts[:open_id_connect_url]
            )
      ...
      def add_paths!
        ...
        examples.each do |example|
          ...
          operation.safe_assign_setting(:summary, example.respond_to?(:summary) ? example.summary : '')
          operation.safe_assign_setting(:description, example.respond_to?(:description) ? example.description : '')
          operation.safe_assign_setting(:externalDocs, example.respond_to?(:external_docs) ? example.external_docs : '')
          ...
          operation.safe_assign_setting(:deprecated, example.respond_to?(:deprecated) ? example.deprecated : false)
        ...
        example.requests.each do |request|
          ...
          if request[:response_headers]
            ...
            request[:response_headers].each do |header, value|
              response.headers[header.to_s] = OpenApi::Header.new(example => value)
            end
          end
          ...
          if /\A(?<response_content_type>[^;]+)/ =~ request[:response_content_type]
            ...
            response.examples[response_content_type.to_s] = OpenApi::Example.new(value: response_body)
          end
          responses.add_setting request[:response_status].to_s, :value => response
        end
      end
      ...
      def extract_schema(fields)
        ...
        fields.each do |field|
          ...
          current[:properties][field[:name]][:example] = field[:value] if field[:value]
